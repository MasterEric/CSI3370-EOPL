...
; This implementation of MUTABLE-PAIRS was completed through a combination of
; the Essentials of Programming Languages textbook and the previous IMPLICIT-REFS language,
...
; Mutable pair stuff from page 125.
...
(define expval->mutpair
  (lambda (val)
    (cases expval val
      (mutpair-val (ref) ref)
      (else (printf "Could not extract mutable pair value from ~s" val)))))
    ...
    (cases expression exp
      ...
      (right-exp (exp1)
        (let ((val1 (value-of exp1 env)))
          (let ((p1 (expval->mutpair val1)))
            (right p1))))
      (setleft-exp (exp-pair exp-val)
        (let ((val-pair (value-of exp-pair env))
              (val-val (value-of exp-val env)))
          (let ((p (expval->mutpair val-pair)))
            (begin
              (setleft p val-val)
              (num-val 82)))))
      (setright-exp (exp-pair exp-val)
        (let ((val-pair (value-of exp-pair env))
              (val-val (value-of exp-val env)))
          (let ((p (expval->mutpair val-pair)))
            (begin
